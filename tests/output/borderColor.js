module.exports = `
  @layer utilities {
    .border-lb-black {
      border-block-start-color: #000;
      border-block-end-color: #000
    }
    .border-lb-current {
      border-block-start-color: currentColor;
      border-block-end-color: currentColor
    }
    .border-lb-gray-100 {
      border-block-start-color: #f3f4f6;
      border-block-end-color: #f3f4f6
    }
    .border-lb-gray-200 {
      border-block-start-color: #e5e7eb;
      border-block-end-color: #e5e7eb
    }
    .border-lb-gray-300 {
      border-block-start-color: #d1d5db;
      border-block-end-color: #d1d5db
    }
    .border-lb-gray-400 {
      border-block-start-color: #9ca3af;
      border-block-end-color: #9ca3af
    }
    .border-lb-gray-50 {
      border-block-start-color: #f9fafb;
      border-block-end-color: #f9fafb
    }
    .border-lb-gray-500 {
      border-block-start-color: #6b7280;
      border-block-end-color: #6b7280
    }
    .border-lb-gray-600 {
      border-block-start-color: #4b5563;
      border-block-end-color: #4b5563
    }
    .border-lb-gray-700 {
      border-block-start-color: #374151;
      border-block-end-color: #374151
    }
    .border-lb-gray-800 {
      border-block-start-color: #1f2937;
      border-block-end-color: #1f2937
    }
    .border-lb-gray-900 {
      border-block-start-color: #111827;
      border-block-end-color: #111827
    }
    .border-lb-transparent {
      border-block-start-color: transparent;
      border-block-end-color: transparent
    }
    .border-lb-white {
      border-block-start-color: #fff;
      border-block-end-color: #fff
    }
    .border-li-black {
      border-inline-start-color: #000;
      border-inline-end-color: #000
    }
    .border-li-current {
      border-inline-start-color: currentColor;
      border-inline-end-color: currentColor
    }
    .border-li-gray-100 {
      border-inline-start-color: #f3f4f6;
      border-inline-end-color: #f3f4f6
    }
    .border-li-gray-200 {
      border-inline-start-color: #e5e7eb;
      border-inline-end-color: #e5e7eb
    }
    .border-li-gray-300 {
      border-inline-start-color: #d1d5db;
      border-inline-end-color: #d1d5db
    }
    .border-li-gray-400 {
      border-inline-start-color: #9ca3af;
      border-inline-end-color: #9ca3af
    }
    .border-li-gray-50 {
      border-inline-start-color: #f9fafb;
      border-inline-end-color: #f9fafb
    }
    .border-li-gray-500 {
      border-inline-start-color: #6b7280;
      border-inline-end-color: #6b7280
    }
    .border-li-gray-600 {
      border-inline-start-color: #4b5563;
      border-inline-end-color: #4b5563
    }
    .border-li-gray-700 {
      border-inline-start-color: #374151;
      border-inline-end-color: #374151
    }
    .border-li-gray-800 {
      border-inline-start-color: #1f2937;
      border-inline-end-color: #1f2937
    }
    .border-li-gray-900 {
      border-inline-start-color: #111827;
      border-inline-end-color: #111827
    }
    .border-li-transparent {
      border-inline-start-color: transparent;
      border-inline-end-color: transparent
    }
    .border-li-white {
      border-inline-start-color: #fff;
      border-inline-end-color: #fff
    }
    .\\!border-bs-black {
      border-block-start-color: #000 !important
    }
    .border-be-black {
      border-block-end-color: #000
    }
    .border-be-blue-100 {
      border-block-end-color: #dbeafe
    }
    .border-be-blue-200 {
      border-block-end-color: #bfdbfe
    }
    .border-be-blue-300 {
      border-block-end-color: #93c5fd
    }
    .border-be-blue-400 {
      border-block-end-color: #60a5fa
    }
    .border-be-blue-50 {
      border-block-end-color: #eff6ff
    }
    .border-be-blue-500 {
      border-block-end-color: #3b82f6
    }
    .border-be-blue-600 {
      border-block-end-color: #2563eb
    }
    .border-be-blue-700 {
      border-block-end-color: #1d4ed8
    }
    .border-be-blue-800 {
      border-block-end-color: #1e40af
    }
    .border-be-blue-900 {
      border-block-end-color: #1e3a8a
    }
    .border-be-current {
      border-block-end-color: currentColor
    }
    .border-be-gray-100 {
      border-block-end-color: #f3f4f6
    }
    .border-be-gray-200 {
      border-block-end-color: #e5e7eb
    }
    .border-be-gray-300 {
      border-block-end-color: #d1d5db
    }
    .border-be-gray-400 {
      border-block-end-color: #9ca3af
    }
    .border-be-gray-50 {
      border-block-end-color: #f9fafb
    }
    .border-be-gray-500 {
      border-block-end-color: #6b7280
    }
    .border-be-gray-600 {
      border-block-end-color: #4b5563
    }
    .border-be-gray-700 {
      border-block-end-color: #374151
    }
    .border-be-gray-800 {
      border-block-end-color: #1f2937
    }
    .border-be-gray-900 {
      border-block-end-color: #111827
    }
    .border-be-green-100 {
      border-block-end-color: #dcfce7
    }
    .border-be-green-200 {
      border-block-end-color: #bbf7d0
    }
    .border-be-green-300 {
      border-block-end-color: #86efac
    }
    .border-be-green-400 {
      border-block-end-color: #4ade80
    }
    .border-be-green-50 {
      border-block-end-color: #f0fdf4
    }
    .border-be-green-500 {
      border-block-end-color: #22c55e
    }
    .border-be-green-600 {
      border-block-end-color: #16a34a
    }
    .border-be-green-700 {
      border-block-end-color: #15803d
    }
    .border-be-green-800 {
      border-block-end-color: #166534
    }
    .border-be-green-900 {
      border-block-end-color: #14532d
    }
    .border-be-indigo-100 {
      border-block-end-color: #e0e7ff
    }
    .border-be-indigo-200 {
      border-block-end-color: #c7d2fe
    }
    .border-be-indigo-300 {
      border-block-end-color: #a5b4fc
    }
    .border-be-indigo-400 {
      border-block-end-color: #818cf8
    }
    .border-be-indigo-50 {
      border-block-end-color: #eef2ff
    }
    .border-be-indigo-500 {
      border-block-end-color: #6366f1
    }
    .border-be-indigo-600 {
      border-block-end-color: #4f46e5
    }
    .border-be-indigo-700 {
      border-block-end-color: #4338ca
    }
    .border-be-indigo-800 {
      border-block-end-color: #3730a3
    }
    .border-be-indigo-900 {
      border-block-end-color: #312e81
    }
    .border-be-pink-100 {
      border-block-end-color: #fce7f3
    }
    .border-be-pink-200 {
      border-block-end-color: #fbcfe8
    }
    .border-be-pink-300 {
      border-block-end-color: #f9a8d4
    }
    .border-be-pink-400 {
      border-block-end-color: #f472b6
    }
    .border-be-pink-50 {
      border-block-end-color: #fdf2f8
    }
    .border-be-pink-500 {
      border-block-end-color: #ec4899
    }
    .border-be-pink-600 {
      border-block-end-color: #db2777
    }
    .border-be-pink-700 {
      border-block-end-color: #be185d
    }
    .border-be-pink-800 {
      border-block-end-color: #9d174d
    }
    .border-be-pink-900 {
      border-block-end-color: #831843
    }
    .border-be-purple-100 {
      border-block-end-color: #f3e8ff
    }
    .border-be-purple-200 {
      border-block-end-color: #e9d5ff
    }
    .border-be-purple-300 {
      border-block-end-color: #d8b4fe
    }
    .border-be-purple-400 {
      border-block-end-color: #c084fc
    }
    .border-be-purple-50 {
      border-block-end-color: #faf5ff
    }
    .border-be-purple-500 {
      border-block-end-color: #a855f7
    }
    .border-be-purple-600 {
      border-block-end-color: #9333ea
    }
    .border-be-purple-700 {
      border-block-end-color: #7e22ce
    }
    .border-be-purple-800 {
      border-block-end-color: #6b21a8
    }
    .border-be-purple-900 {
      border-block-end-color: #581c87
    }
    .border-be-red-100 {
      border-block-end-color: #fee2e2
    }
    .border-be-red-200 {
      border-block-end-color: #fecaca
    }
    .border-be-red-300 {
      border-block-end-color: #fca5a5
    }
    .border-be-red-400 {
      border-block-end-color: #f87171
    }
    .border-be-red-50 {
      border-block-end-color: #fef2f2
    }
    .border-be-red-500 {
      border-block-end-color: #ef4444
    }
    .border-be-red-600 {
      border-block-end-color: #dc2626
    }
    .border-be-red-700 {
      border-block-end-color: #b91c1c
    }
    .border-be-red-800 {
      border-block-end-color: #991b1b
    }
    .border-be-red-900 {
      border-block-end-color: #7f1d1d
    }
    .border-be-transparent {
      border-block-end-color: transparent
    }
    .border-be-white {
      border-block-end-color: #fff
    }
    .border-be-yellow-100 {
      border-block-end-color: #fef9c3
    }
    .border-be-yellow-200 {
      border-block-end-color: #fef08a
    }
    .border-be-yellow-300 {
      border-block-end-color: #fde047
    }
    .border-be-yellow-400 {
      border-block-end-color: #facc15
    }
    .border-be-yellow-50 {
      border-block-end-color: #fefce8
    }
    .border-be-yellow-500 {
      border-block-end-color: #eab308
    }
    .border-be-yellow-600 {
      border-block-end-color: #ca8a04
    }
    .border-be-yellow-700 {
      border-block-end-color: #a16207
    }
    .border-be-yellow-800 {
      border-block-end-color: #854d0e
    }
    .border-be-yellow-900 {
      border-block-end-color: #713f12
    }
    .border-bs-black {
      border-block-start-color: #000
    }
    .border-bs-black\\/25 {
      border-block-start-color: rgb(0 0 0 / 0.25)
    }
    .border-bs-blue-100 {
      border-block-start-color: #dbeafe
    }
    .border-bs-blue-200 {
      border-block-start-color: #bfdbfe
    }
    .border-bs-blue-300 {
      border-block-start-color: #93c5fd
    }
    .border-bs-blue-400 {
      border-block-start-color: #60a5fa
    }
    .border-bs-blue-50 {
      border-block-start-color: #eff6ff
    }
    .border-bs-blue-500 {
      border-block-start-color: #3b82f6
    }
    .border-bs-blue-600 {
      border-block-start-color: #2563eb
    }
    .border-bs-blue-700 {
      border-block-start-color: #1d4ed8
    }
    .border-bs-blue-800 {
      border-block-start-color: #1e40af
    }
    .border-bs-blue-900 {
      border-block-start-color: #1e3a8a
    }
    .border-bs-current {
      border-block-start-color: currentColor
    }
    .border-bs-gray-100 {
      border-block-start-color: #f3f4f6
    }
    .border-bs-gray-200 {
      border-block-start-color: #e5e7eb
    }
    .border-bs-gray-300 {
      border-block-start-color: #d1d5db
    }
    .border-bs-gray-400 {
      border-block-start-color: #9ca3af
    }
    .border-bs-gray-50 {
      border-block-start-color: #f9fafb
    }
    .border-bs-gray-500 {
      border-block-start-color: #6b7280
    }
    .border-bs-gray-600 {
      border-block-start-color: #4b5563
    }
    .border-bs-gray-700 {
      border-block-start-color: #374151
    }
    .border-bs-gray-800 {
      border-block-start-color: #1f2937
    }
    .border-bs-gray-900 {
      border-block-start-color: #111827
    }
    .border-bs-green-100 {
      border-block-start-color: #dcfce7
    }
    .border-bs-green-200 {
      border-block-start-color: #bbf7d0
    }
    .border-bs-green-300 {
      border-block-start-color: #86efac
    }
    .border-bs-green-400 {
      border-block-start-color: #4ade80
    }
    .border-bs-green-50 {
      border-block-start-color: #f0fdf4
    }
    .border-bs-green-500 {
      border-block-start-color: #22c55e
    }
    .border-bs-green-600 {
      border-block-start-color: #16a34a
    }
    .border-bs-green-700 {
      border-block-start-color: #15803d
    }
    .border-bs-green-800 {
      border-block-start-color: #166534
    }
    .border-bs-green-900 {
      border-block-start-color: #14532d
    }
    .border-bs-indigo-100 {
      border-block-start-color: #e0e7ff
    }
    .border-bs-indigo-200 {
      border-block-start-color: #c7d2fe
    }
    .border-bs-indigo-300 {
      border-block-start-color: #a5b4fc
    }
    .border-bs-indigo-400 {
      border-block-start-color: #818cf8
    }
    .border-bs-indigo-50 {
      border-block-start-color: #eef2ff
    }
    .border-bs-indigo-500 {
      border-block-start-color: #6366f1
    }
    .border-bs-indigo-600 {
      border-block-start-color: #4f46e5
    }
    .border-bs-indigo-700 {
      border-block-start-color: #4338ca
    }
    .border-bs-indigo-800 {
      border-block-start-color: #3730a3
    }
    .border-bs-indigo-900 {
      border-block-start-color: #312e81
    }
    .border-bs-pink-100 {
      border-block-start-color: #fce7f3
    }
    .border-bs-pink-200 {
      border-block-start-color: #fbcfe8
    }
    .border-bs-pink-300 {
      border-block-start-color: #f9a8d4
    }
    .border-bs-pink-400 {
      border-block-start-color: #f472b6
    }
    .border-bs-pink-50 {
      border-block-start-color: #fdf2f8
    }
    .border-bs-pink-500 {
      border-block-start-color: #ec4899
    }
    .border-bs-pink-600 {
      border-block-start-color: #db2777
    }
    .border-bs-pink-700 {
      border-block-start-color: #be185d
    }
    .border-bs-pink-800 {
      border-block-start-color: #9d174d
    }
    .border-bs-pink-900 {
      border-block-start-color: #831843
    }
    .border-bs-purple-100 {
      border-block-start-color: #f3e8ff
    }
    .border-bs-purple-200 {
      border-block-start-color: #e9d5ff
    }
    .border-bs-purple-300 {
      border-block-start-color: #d8b4fe
    }
    .border-bs-purple-400 {
      border-block-start-color: #c084fc
    }
    .border-bs-purple-50 {
      border-block-start-color: #faf5ff
    }
    .border-bs-purple-500 {
      border-block-start-color: #a855f7
    }
    .border-bs-purple-600 {
      border-block-start-color: #9333ea
    }
    .border-bs-purple-700 {
      border-block-start-color: #7e22ce
    }
    .border-bs-purple-800 {
      border-block-start-color: #6b21a8
    }
    .border-bs-purple-900 {
      border-block-start-color: #581c87
    }
    .border-bs-red-100 {
      border-block-start-color: #fee2e2
    }
    .border-bs-red-200 {
      border-block-start-color: #fecaca
    }
    .border-bs-red-300 {
      border-block-start-color: #fca5a5
    }
    .border-bs-red-400 {
      border-block-start-color: #f87171
    }
    .border-bs-red-50 {
      border-block-start-color: #fef2f2
    }
    .border-bs-red-500 {
      border-block-start-color: #ef4444
    }
    .border-bs-red-600 {
      border-block-start-color: #dc2626
    }
    .border-bs-red-700 {
      border-block-start-color: #b91c1c
    }
    .border-bs-red-800 {
      border-block-start-color: #991b1b
    }
    .border-bs-red-900 {
      border-block-start-color: #7f1d1d
    }
    .border-bs-transparent {
      border-block-start-color: transparent
    }
    .border-bs-white {
      border-block-start-color: #fff
    }
    .border-bs-yellow-100 {
      border-block-start-color: #fef9c3
    }
    .border-bs-yellow-200 {
      border-block-start-color: #fef08a
    }
    .border-bs-yellow-300 {
      border-block-start-color: #fde047
    }
    .border-bs-yellow-400 {
      border-block-start-color: #facc15
    }
    .border-bs-yellow-50 {
      border-block-start-color: #fefce8
    }
    .border-bs-yellow-500 {
      border-block-start-color: #eab308
    }
    .border-bs-yellow-600 {
      border-block-start-color: #ca8a04
    }
    .border-bs-yellow-700 {
      border-block-start-color: #a16207
    }
    .border-bs-yellow-800 {
      border-block-start-color: #854d0e
    }
    .border-bs-yellow-900 {
      border-block-start-color: #713f12
    }
    .border-ie-black {
      border-inline-end-color: #000
    }
    .border-ie-blue-100 {
      border-inline-end-color: #dbeafe
    }
    .border-ie-blue-200 {
      border-inline-end-color: #bfdbfe
    }
    .border-ie-blue-300 {
      border-inline-end-color: #93c5fd
    }
    .border-ie-blue-400 {
      border-inline-end-color: #60a5fa
    }
    .border-ie-blue-50 {
      border-inline-end-color: #eff6ff
    }
    .border-ie-blue-500 {
      border-inline-end-color: #3b82f6
    }
    .border-ie-blue-600 {
      border-inline-end-color: #2563eb
    }
    .border-ie-blue-700 {
      border-inline-end-color: #1d4ed8
    }
    .border-ie-blue-800 {
      border-inline-end-color: #1e40af
    }
    .border-ie-blue-900 {
      border-inline-end-color: #1e3a8a
    }
    .border-ie-current {
      border-inline-end-color: currentColor
    }
    .border-ie-gray-100 {
      border-inline-end-color: #f3f4f6
    }
    .border-ie-gray-200 {
      border-inline-end-color: #e5e7eb
    }
    .border-ie-gray-300 {
      border-inline-end-color: #d1d5db
    }
    .border-ie-gray-400 {
      border-inline-end-color: #9ca3af
    }
    .border-ie-gray-50 {
      border-inline-end-color: #f9fafb
    }
    .border-ie-gray-500 {
      border-inline-end-color: #6b7280
    }
    .border-ie-gray-600 {
      border-inline-end-color: #4b5563
    }
    .border-ie-gray-700 {
      border-inline-end-color: #374151
    }
    .border-ie-gray-800 {
      border-inline-end-color: #1f2937
    }
    .border-ie-gray-900 {
      border-inline-end-color: #111827
    }
    .border-ie-green-100 {
      border-inline-end-color: #dcfce7
    }
    .border-ie-green-200 {
      border-inline-end-color: #bbf7d0
    }
    .border-ie-green-300 {
      border-inline-end-color: #86efac
    }
    .border-ie-green-400 {
      border-inline-end-color: #4ade80
    }
    .border-ie-green-50 {
      border-inline-end-color: #f0fdf4
    }
    .border-ie-green-500 {
      border-inline-end-color: #22c55e
    }
    .border-ie-green-600 {
      border-inline-end-color: #16a34a
    }
    .border-ie-green-700 {
      border-inline-end-color: #15803d
    }
    .border-ie-green-800 {
      border-inline-end-color: #166534
    }
    .border-ie-green-900 {
      border-inline-end-color: #14532d
    }
    .border-ie-indigo-100 {
      border-inline-end-color: #e0e7ff
    }
    .border-ie-indigo-200 {
      border-inline-end-color: #c7d2fe
    }
    .border-ie-indigo-300 {
      border-inline-end-color: #a5b4fc
    }
    .border-ie-indigo-400 {
      border-inline-end-color: #818cf8
    }
    .border-ie-indigo-50 {
      border-inline-end-color: #eef2ff
    }
    .border-ie-indigo-500 {
      border-inline-end-color: #6366f1
    }
    .border-ie-indigo-600 {
      border-inline-end-color: #4f46e5
    }
    .border-ie-indigo-700 {
      border-inline-end-color: #4338ca
    }
    .border-ie-indigo-800 {
      border-inline-end-color: #3730a3
    }
    .border-ie-indigo-900 {
      border-inline-end-color: #312e81
    }
    .border-ie-pink-100 {
      border-inline-end-color: #fce7f3
    }
    .border-ie-pink-200 {
      border-inline-end-color: #fbcfe8
    }
    .border-ie-pink-300 {
      border-inline-end-color: #f9a8d4
    }
    .border-ie-pink-400 {
      border-inline-end-color: #f472b6
    }
    .border-ie-pink-50 {
      border-inline-end-color: #fdf2f8
    }
    .border-ie-pink-500 {
      border-inline-end-color: #ec4899
    }
    .border-ie-pink-600 {
      border-inline-end-color: #db2777
    }
    .border-ie-pink-700 {
      border-inline-end-color: #be185d
    }
    .border-ie-pink-800 {
      border-inline-end-color: #9d174d
    }
    .border-ie-pink-900 {
      border-inline-end-color: #831843
    }
    .border-ie-purple-100 {
      border-inline-end-color: #f3e8ff
    }
    .border-ie-purple-200 {
      border-inline-end-color: #e9d5ff
    }
    .border-ie-purple-300 {
      border-inline-end-color: #d8b4fe
    }
    .border-ie-purple-400 {
      border-inline-end-color: #c084fc
    }
    .border-ie-purple-50 {
      border-inline-end-color: #faf5ff
    }
    .border-ie-purple-500 {
      border-inline-end-color: #a855f7
    }
    .border-ie-purple-600 {
      border-inline-end-color: #9333ea
    }
    .border-ie-purple-700 {
      border-inline-end-color: #7e22ce
    }
    .border-ie-purple-800 {
      border-inline-end-color: #6b21a8
    }
    .border-ie-purple-900 {
      border-inline-end-color: #581c87
    }
    .border-ie-red-100 {
      border-inline-end-color: #fee2e2
    }
    .border-ie-red-200 {
      border-inline-end-color: #fecaca
    }
    .border-ie-red-300 {
      border-inline-end-color: #fca5a5
    }
    .border-ie-red-400 {
      border-inline-end-color: #f87171
    }
    .border-ie-red-50 {
      border-inline-end-color: #fef2f2
    }
    .border-ie-red-500 {
      border-inline-end-color: #ef4444
    }
    .border-ie-red-600 {
      border-inline-end-color: #dc2626
    }
    .border-ie-red-700 {
      border-inline-end-color: #b91c1c
    }
    .border-ie-red-800 {
      border-inline-end-color: #991b1b
    }
    .border-ie-red-900 {
      border-inline-end-color: #7f1d1d
    }
    .border-ie-transparent {
      border-inline-end-color: transparent
    }
    .border-ie-white {
      border-inline-end-color: #fff
    }
    .border-ie-yellow-100 {
      border-inline-end-color: #fef9c3
    }
    .border-ie-yellow-200 {
      border-inline-end-color: #fef08a
    }
    .border-ie-yellow-300 {
      border-inline-end-color: #fde047
    }
    .border-ie-yellow-400 {
      border-inline-end-color: #facc15
    }
    .border-ie-yellow-50 {
      border-inline-end-color: #fefce8
    }
    .border-ie-yellow-500 {
      border-inline-end-color: #eab308
    }
    .border-ie-yellow-600 {
      border-inline-end-color: #ca8a04
    }
    .border-ie-yellow-700 {
      border-inline-end-color: #a16207
    }
    .border-ie-yellow-800 {
      border-inline-end-color: #854d0e
    }
    .border-ie-yellow-900 {
      border-inline-end-color: #713f12
    }
    .border-is-black {
      border-inline-start-color: #000
    }
    .border-is-blue-100 {
      border-inline-start-color: #dbeafe
    }
    .border-is-blue-200 {
      border-inline-start-color: #bfdbfe
    }
    .border-is-blue-300 {
      border-inline-start-color: #93c5fd
    }
    .border-is-blue-400 {
      border-inline-start-color: #60a5fa
    }
    .border-is-blue-50 {
      border-inline-start-color: #eff6ff
    }
    .border-is-blue-500 {
      border-inline-start-color: #3b82f6
    }
    .border-is-blue-600 {
      border-inline-start-color: #2563eb
    }
    .border-is-blue-700 {
      border-inline-start-color: #1d4ed8
    }
    .border-is-blue-800 {
      border-inline-start-color: #1e40af
    }
    .border-is-blue-900 {
      border-inline-start-color: #1e3a8a
    }
    .border-is-current {
      border-inline-start-color: currentColor
    }
    .border-is-gray-100 {
      border-inline-start-color: #f3f4f6
    }
    .border-is-gray-200 {
      border-inline-start-color: #e5e7eb
    }
    .border-is-gray-300 {
      border-inline-start-color: #d1d5db
    }
    .border-is-gray-400 {
      border-inline-start-color: #9ca3af
    }
    .border-is-gray-50 {
      border-inline-start-color: #f9fafb
    }
    .border-is-gray-500 {
      border-inline-start-color: #6b7280
    }
    .border-is-gray-600 {
      border-inline-start-color: #4b5563
    }
    .border-is-gray-700 {
      border-inline-start-color: #374151
    }
    .border-is-gray-800 {
      border-inline-start-color: #1f2937
    }
    .border-is-gray-900 {
      border-inline-start-color: #111827
    }
    .border-is-green-100 {
      border-inline-start-color: #dcfce7
    }
    .border-is-green-200 {
      border-inline-start-color: #bbf7d0
    }
    .border-is-green-300 {
      border-inline-start-color: #86efac
    }
    .border-is-green-400 {
      border-inline-start-color: #4ade80
    }
    .border-is-green-50 {
      border-inline-start-color: #f0fdf4
    }
    .border-is-green-500 {
      border-inline-start-color: #22c55e
    }
    .border-is-green-600 {
      border-inline-start-color: #16a34a
    }
    .border-is-green-700 {
      border-inline-start-color: #15803d
    }
    .border-is-green-800 {
      border-inline-start-color: #166534
    }
    .border-is-green-900 {
      border-inline-start-color: #14532d
    }
    .border-is-indigo-100 {
      border-inline-start-color: #e0e7ff
    }
    .border-is-indigo-200 {
      border-inline-start-color: #c7d2fe
    }
    .border-is-indigo-300 {
      border-inline-start-color: #a5b4fc
    }
    .border-is-indigo-400 {
      border-inline-start-color: #818cf8
    }
    .border-is-indigo-50 {
      border-inline-start-color: #eef2ff
    }
    .border-is-indigo-500 {
      border-inline-start-color: #6366f1
    }
    .border-is-indigo-600 {
      border-inline-start-color: #4f46e5
    }
    .border-is-indigo-700 {
      border-inline-start-color: #4338ca
    }
    .border-is-indigo-800 {
      border-inline-start-color: #3730a3
    }
    .border-is-indigo-900 {
      border-inline-start-color: #312e81
    }
    .border-is-pink-100 {
      border-inline-start-color: #fce7f3
    }
    .border-is-pink-200 {
      border-inline-start-color: #fbcfe8
    }
    .border-is-pink-300 {
      border-inline-start-color: #f9a8d4
    }
    .border-is-pink-400 {
      border-inline-start-color: #f472b6
    }
    .border-is-pink-50 {
      border-inline-start-color: #fdf2f8
    }
    .border-is-pink-500 {
      border-inline-start-color: #ec4899
    }
    .border-is-pink-600 {
      border-inline-start-color: #db2777
    }
    .border-is-pink-700 {
      border-inline-start-color: #be185d
    }
    .border-is-pink-800 {
      border-inline-start-color: #9d174d
    }
    .border-is-pink-900 {
      border-inline-start-color: #831843
    }
    .border-is-purple-100 {
      border-inline-start-color: #f3e8ff
    }
    .border-is-purple-200 {
      border-inline-start-color: #e9d5ff
    }
    .border-is-purple-300 {
      border-inline-start-color: #d8b4fe
    }
    .border-is-purple-400 {
      border-inline-start-color: #c084fc
    }
    .border-is-purple-50 {
      border-inline-start-color: #faf5ff
    }
    .border-is-purple-500 {
      border-inline-start-color: #a855f7
    }
    .border-is-purple-600 {
      border-inline-start-color: #9333ea
    }
    .border-is-purple-700 {
      border-inline-start-color: #7e22ce
    }
    .border-is-purple-800 {
      border-inline-start-color: #6b21a8
    }
    .border-is-purple-900 {
      border-inline-start-color: #581c87
    }
    .border-is-red-100 {
      border-inline-start-color: #fee2e2
    }
    .border-is-red-200 {
      border-inline-start-color: #fecaca
    }
    .border-is-red-300 {
      border-inline-start-color: #fca5a5
    }
    .border-is-red-400 {
      border-inline-start-color: #f87171
    }
    .border-is-red-50 {
      border-inline-start-color: #fef2f2
    }
    .border-is-red-500 {
      border-inline-start-color: #ef4444
    }
    .border-is-red-600 {
      border-inline-start-color: #dc2626
    }
    .border-is-red-700 {
      border-inline-start-color: #b91c1c
    }
    .border-is-red-800 {
      border-inline-start-color: #991b1b
    }
    .border-is-red-900 {
      border-inline-start-color: #7f1d1d
    }
    .border-is-transparent {
      border-inline-start-color: transparent
    }
    .border-is-white {
      border-inline-start-color: #fff
    }
    .border-is-yellow-100 {
      border-inline-start-color: #fef9c3
    }
    .border-is-yellow-200 {
      border-inline-start-color: #fef08a
    }
    .border-is-yellow-300 {
      border-inline-start-color: #fde047
    }
    .border-is-yellow-400 {
      border-inline-start-color: #facc15
    }
    .border-is-yellow-50 {
      border-inline-start-color: #fefce8
    }
    .border-is-yellow-500 {
      border-inline-start-color: #eab308
    }
    .border-is-yellow-600 {
      border-inline-start-color: #ca8a04
    }
    .border-is-yellow-700 {
      border-inline-start-color: #a16207
    }
    .border-is-yellow-800 {
      border-inline-start-color: #854d0e
    }
    .border-is-yellow-900 {
      border-inline-start-color: #713f12
    }
  }
  .before\\:border-bs-black::before {
    content: var(--tw-content);
    border-block-start-color: #000
  }
  .only\\:border-bs-black:only-child {
    border-block-start-color: #000
  }
  .disabled\\:border-bs-black:disabled {
    border-block-start-color: #000
  }
  .peer:checked ~ .peer-checked\\:border-bs-black {
    border-block-start-color: #000
  }
  @media (min-width: 768px) {
    .md\\:disabled\\:focus\\:hover\\:border-bs-black:hover:focus:disabled {
      border-block-start-color: #000
    }
  }
  @media (prefers-color-scheme: dark) {
    .dark\\:border-be-black {
      border-block-end-color: #000
    }
    .dark\\:border-bs-black {
        border-block-start-color: #000
    }
    .dark\\:border-ie-black {
      border-inline-end-color: #000
    }
    .dark\\:border-is-black {
        border-inline-start-color: #000
    }
  }
  @media (min-width: 768px) {
    @media (prefers-color-scheme: dark) {
        .md\\:dark\\:disabled\\:focus\\:hover\\:border-bs-black:hover:focus:disabled {
          border-block-start-color: #000
        }
    }
  }
`;
